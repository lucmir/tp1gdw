...
//    iterator find_subtree(sibling_iterator, sibling_iterator, iterator from, iterator to, BinaryPredicate) const;
      ...
      //void virtual_merge(tree<T> &tr1, tree<T> &tr2); 
...
/*
template <class T, class tree_node_allocator>
void tree<T, tree_node_allocator>::virtual_merge(tree<T> &tr1, tree<T> &tr2) 
   { 
   //head = alloc_.allocate(1,0); // MSVC does not have default second argument 
   //feet = alloc_.allocate(1,0);

   head->parent=0;
   //head->first_child= tr1.head->first_child;
   head->last_child->next_sibling = tr2.head->first_child;
   tr2.head->first_child->prev_sibling = head->last_child;
   //head->prev_sibling=0; //head;
   //head->next_sibling = head;
   head->last_child = tr2.head->first_child;
   tr2.head->first_child->parent = head;
